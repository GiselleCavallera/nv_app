    ...
    var route = 'aa';
        $.post(route, data, function(result) {
            ...
            if(result.dia != "DD/MM/AAAA" && result.inscripto == true)
            {
                ...
                $('#c_{{$taller->id}}').bootstrapToggle('on'); //nombre del botón//$('#c_{{$taller->id}}').attr('checked', true);//= true;          //
            }
            else{
                if(result.dia != "DD/MM/AAAA" && result.inscripto == false)
                    $('.s_{{$taller->id}}').val(result.dia+result.hora);
                    $('#c_{{$taller->id}}').bootstrapToggle('off');//$('#c_{{$taller->id}}').attr('checked', false);//$('#c_{{$taller->id}}').checked= false;  //
            }
...
//por cada taller recorrer los dictados y activar solo el que esta seleccionado
                ...
                <div class="panel-body">
                        ...
                        <form class="form-horizontal"  id="form_{{$taller->id}}" method="POST" action="{{ route('inscripciones') }}">
                            ...
                            <div class="row" id="{{$taller->id}}" style="background-color: #3097D1; color: white; text-align: center">  
                                <div style="text-align:center; font-size: 1.1em; display:inline-block">{{$taller->titulo}}</div> <!-- gc 21/7/17 -->
                            ...
                            <div class="row o_{{$taller->id}}" style="background-color: #F8F8FF">
                                ...
                                <div class="col-xs-8">
                                    <div class="row" style="font-size: 0.8em;">Duración: {{$taller->duracion}} min.(aprox.)</div>
                                    ...
                                    <!--<div class="row" style="border: solid green 1px">
                                        @foreach( $dictados as $key => $dictado)
                                            @if($dictado->id_taller == $taller->id)
                                                <div id="h_{{$taller->id}}"><h2>{{$dictado->sala}}</h2></div>
                                            @endIf
                                        @endforeach
                                    </div>
                                         ...
                                         <div class="xs-col-6">
                                            <div class="checkbox-lg">
                                                <label style="font-size: 27px">
                                                    <input type="checkbox" data-toggle="toggle" id="c_{{$taller->id}}" onclick="validar({{$taller->id}})" name="c_{{$taller->id}}" data-on="Inscripto" data-off="No Inscripto" >
